...
describe('mcp tools coverage', () => {
    ...
    it('covers FR-039 tool schemas enforce required fields for canonical and compatibility names', () => {
        ...
        expect(TOOL_SCHEMAS.agirunner_create_task.safeParse({ title: 'x', type: 'code' }).success).toBe(true);
        ...
        expect(TOOL_SCHEMAS.agirunner_create_workflow.safeParse({ name: 'pipe', playbook_id: 'pb-1' })
            .success).toBe(true);
        expect(TOOL_SCHEMAS.create_workflow_work_item.safeParse({
            workflow_id: 'wf-1',
            title: 'Work',
        }).success).toBe(true);
        expect(TOOL_SCHEMAS.agirunner_update_workflow_work_item.safeParse({
            workflow_id: 'wf-1',
            work_item_id: 'wi-1',
        }).success).toBe(true);
    ...
    it('covers FR-038/FR-039 handlers delegate operations to sdk client (canonical + compatibility aliases)', async () => {
        ...
        expect(client.createWorkflowWorkItem).toHaveBeenCalledWith('p1', expect.objectContaining({ title: 'Investigate' }));
        expect(client.updateWorkflowWorkItem).toHaveBeenCalledWith('p1', 'wi-1', expect.objectContaining({ priority: 'high' }));
