...
export function createToolHandlers(client) {
  const canonicalHandlers = {
    ...
    baton_claim_task: (input) => client.claimTask({ agent_id: String(input.agent_id), capabilities: Array.isArray(input.capabilities) ? input.capabilities : [] }),
    baton_complete_task: (input) => client.completeTask(String(input.id), input.output ?? {}),
    baton_list_pipelines: (input) => client.listPipelines(input),
    baton_create_pipeline: (input) => client.createPipeline(input),
    baton_cancel_pipeline: (input) => client.cancelPipeline(String(input.id)),
  };
