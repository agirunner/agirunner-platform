...
describe('environment override precedence (FR-295)', () => {
  it('applies environment values over file configuration', () => {
    ...
    const config = loadWorkerConfig({
      filePath,
      env: {
        AGENTBATON_WORKER_SERVER_URL: 'https://env.example.test',
        AGENTBATON_WORKER_AUTH_TOKEN: 'env-token',
        AGENTBATON_WORKER_RUNTIME_ADAPTER: 'openclaw',
        AGENTBATON_WORKER_CAPABILITIES: 'env-a, env-b',
        AGENTBATON_WORKER_TOOL_TAGS_REQUIRED: 'git, ci',
        AGENTBATON_WORKER_TOOL_TAGS_OPTIONAL: 'docker',
        AGENTBATON_WORKER_CONNECTION_MODE: 'websocket',
        AGENTBATON_WORKER_HEARTBEAT_INTERVAL_SECONDS: '12',
        AGENTBATON_WORKER_RECONNECT_MIN_MS: '1500',
        AGENTBATON_WORKER_RECONNECT_MAX_MS: '3500',
        AGENTBATON_WORKER_FILTER_PROJECT_ID: 'project-env',
        AGENTBATON_WORKER_FILTER_TASK_TYPES: 'code,test',
        AGENTBATON_WORKER_LOG_LEVEL: 'error',
      },
    });
