  ...
  it('covers workspace, workflow, document, and artifact parity methods', async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ data: { id: 'workspace-1', memory: { last_run_summary: {} } } }), {
          status: 200,
        }),
      )
      .mockResolvedValueOnce(
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ data: { workflow_id: 'pipe-1', resolved_config: { retries: 2 } } }), {
          status: 200,
        }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ data: [{ logical_name: 'brief', scope: 'workspace', source: 'repository', metadata: {} }] }), {
          status: 200,
        }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ data: { logical_name: 'brief', scope: 'workflow', source: 'external', metadata: {} } }), {
          status: 201,
        }),
      )
      .mockResolvedValueOnce(
