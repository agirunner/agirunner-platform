...
describe('sdk full client coverage', () => {
  it('covers FR-041 typed client methods for task/workflow/agent/worker APIs', async () => {
    const typedStage: WorkflowStage = {
      id: 'stage-typed',
  ...
  it('covers workspace, workflow, document, and artifact parity methods through sdk wrappers', async () => {
    const fetcher = vi
      .fn()
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ data: [{ id: 'workspace-1' }] }), { status: 200 }),
      )
      .mockResolvedValueOnce(
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ data: { id: 'workspace-1', memory: { last_run_summary: {} } } }),
          { status: 200 },
        ),
      )
      .mockResolvedValueOnce(
  ...
  it('covers v2 playbook, board, stage, activation, approval, and task-scope helpers', async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ data: { id: 'workspace-1', memory: { architecture: 'v2' } } }),
          { status: 200 },
        ),
      )
      .mockResolvedValueOnce(
