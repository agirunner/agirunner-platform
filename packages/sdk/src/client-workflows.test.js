...
describe('sdk client workflow wrappers', () => {
    it('covers playbook, board, stage, activation, approval, and task-scope helpers', async () => {
        const fetcher = vi
            ...
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: { columns: [{ id: 'todo', name: 'todo', title: 'To Do', position: 1 }], work_items: [], stage_summary: [] } }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: [{ id: 'stage-1', name: 'build', position: 1, goal: 'Build', status: 'active', gate_status: 'none', iteration_count: 0 }] }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: [{ id: 'item-1', workflow_id: 'wf-1', parent_work_item_id: null, stage_name: 'build', title: 'Implement feature', column_id: 'todo', priority: 'normal', children_count: 2, is_milestone: true, children: [{ id: 'item-1a', workflow_id: 'wf-1', parent_work_item_id: 'item-1', stage_name: 'build', title: 'Implement backend', column_id: 'todo', priority: 'normal', children_count: 0, is_milestone: false }] }] }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: { id: 'item-1', workflow_id: 'wf-1', parent_work_item_id: null, stage_name: 'build', title: 'Implement feature', column_id: 'todo', priority: 'normal', children_count: 2, is_milestone: true, children: [{ id: 'item-1a', workflow_id: 'wf-1', parent_work_item_id: 'item-1', stage_name: 'build', title: 'Implement backend', column_id: 'todo', priority: 'normal', children_count: 0, is_milestone: false }] } }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: [{ id: 'task-1', workflow_id: 'wf-1', work_item_id: 'item-1', title: 'Implement feature', state: 'ready', role: 'developer', stage_name: 'build', activation_id: 'activation-1', is_orchestrator_task: false, created_at: '2026-03-11T00:00:00Z', completed_at: null, depends_on: [] }] }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: [{ id: '1', tenant_id: 'tenant-1', entity_type: 'work_item', entity_id: 'item-1', type: 'work_item.updated', payload: { workflow_id: 'wf-1', work_item_id: 'item-1' }, created_at: '2026-03-11T00:10:00Z' }] }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: { id: 'item-1', workflow_id: 'wf-1', stage_name: 'build', title: 'Implement feature', column_id: 'todo', priority: 'high', notes: 'Updated' } }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: { id: 'item-2', workflow_id: 'wf-1', stage_name: 'build', title: 'Review docs', column_id: 'todo', priority: 'high' } }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: [{ id: 'activation-1', activation_id: 'activation-batch-1', workflow_id: 'wf-1', reason: 'work_item.created', event_type: 'work_item.created', payload: {}, state: 'queued', queued_at: '2026-03-11T00:00:00Z', recovery_status: 'stale_detected', recovery_reason: 'orchestrator task heartbeat expired', recovery_detected_at: '2026-03-11T00:05:00Z', stale_started_at: '2026-03-11T00:03:00Z', redispatched_task_id: 'task-redispatch-1', latest_event_at: '2026-03-11T00:05:00Z', event_count: 2, events: [{ id: 'activation-event-1', activation_id: 'activation-batch-1', reason: 'work_item.created', event_type: 'work_item.created', payload: {}, state: 'queued', queued_at: '2026-03-11T00:00:00Z' }] }] }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: { task_approvals: [{ id: 'task-1', title: 'Review output', state: 'output_pending_assessment', created_at: '2026-03-11T00:00:00Z' }], stage_gates: [] } }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: { memory: { architecture: 'v2' } } }), { status: 200 }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: { id: 'workspace-1', memory: { architecture: 'v2' } } }), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(JSON.stringify({ data: [{ id: 'artifact-2', task_id: 'task-1', name: 'design.md', content_type: 'text/markdown', size_bytes: 128, created_at: '2026-03-11T00:00:00Z', metadata: {}, download_url: '/api/v1/tasks/task-1/artifact-catalog/artifact-2' }] }), {
            status: 200,
        }));
