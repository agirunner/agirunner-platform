...
func TestLogEmitter_EntryFieldsPopulated(t *testing.T) {
	...
	if entry.Metadata["template_id"] != "tmpl-1" {
		t.Errorf("expected metadata template_id=tmpl-1, got %v", entry.Metadata["template_id"])
	}
}
...
func TestReconcile_ContainerCreate_EmitsLog(t *testing.T) {
	...
	for _, e := range entries {
		if e.Operation == "container.create" && e.Status == "completed" {
			found = true
			if e.Metadata["template_id"] != "tmpl-1" {
				t.Errorf("expected template_id tmpl-1, got %v", e.Metadata["template_id"])
			}
			break
		}
	}
...
func TestReconcile_ContainerDestroy_EmitsLog(t *testing.T) {
	...
	for _, e := range entries {
		if e.Operation == "container.destroy" && e.Status == "completed" {
			found = true
			if e.Metadata["container_id"] != "c-1" {
				t.Errorf("expected container_id c-1, got %v", e.Metadata["container_id"])
			}
			break
		}
	}
...
func TestReconcile_HungDetected_EmitsLog(t *testing.T) {
	...
	for _, e := range entries {
		if e.Operation == "container.hung_detected" && e.Level == "warn" {
			found = true
			if e.Metadata["runtime_id"] != "rt-1" {
				t.Errorf("expected runtime_id rt-1, got %v", e.Metadata["runtime_id"])
			}
			break
		}
	}
