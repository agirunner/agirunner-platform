...
func TestRecordScalingEventIncrementsCounter(t *testing.T) {
	...
	created := findCounterValue(mf, map[string]string{"template_id": "tmpl-1", "action": "created"})
	if created != 2 {
		t.Errorf("expected 2 created events for tmpl-1, got %v", created)
	}

	destroyed := findCounterValue(mf, map[string]string{"template_id": "tmpl-2", "action": "destroyed"})
	if destroyed != 1 {
...
func TestUpdateRuntimeGaugesSetsCorrectValues(t *testing.T) {
	...
	containers := []ContainerInfo{
		{ID: "c-1", Labels: map[string]string{labelDCMTemplateID: "tmpl-1", labelDCMRuntimeID: "rt-1"}},
		{ID: "c-2", Labels: map[string]string{labelDCMTemplateID: "tmpl-1", labelDCMRuntimeID: "rt-2"}},
		{ID: "c-3", Labels: map[string]string{labelDCMTemplateID: "tmpl-1", labelDCMRuntimeID: "rt-3", labelDCMDraining: "true"}},
		{ID: "c-4", Labels: map[string]string{labelDCMTemplateID: "tmpl-2", labelDCMRuntimeID: "rt-4"}},
	}
	...
	executing := findGaugeValue(mf, map[string]string{"template_id": "tmpl-1", "state": "executing"})
	if executing != 1 {
		t.Errorf("expected 1 executing for tmpl-1, got %v", executing)
	}

	idle := findGaugeValue(mf, map[string]string{"template_id": "tmpl-1", "state": "idle"})
	if idle != 1 {
		t.Errorf("expected 1 idle for tmpl-1, got %v", idle)
	}

	draining := findGaugeValue(mf, map[string]string{"template_id": "tmpl-1", "state": "draining"})
	if draining != 1 {
		t.Errorf("expected 1 draining for tmpl-1, got %v", draining)
	}

	// rt-4 has no heartbeat data, should be classified as "active"
	active := findGaugeValue(mf, map[string]string{"template_id": "tmpl-2", "state": "active"})
	if active != 1 {
...
func TestSetRuntimeGaugeSetsExplicitValue(t *testing.T) {
	...
	value := findGaugeValue(mf, map[string]string{"template_id": "tmpl-5", "state": "idle"})
	if value != 42 {
...
func TestScalingEventPreemptedAction(t *testing.T) {
	...
	value := findCounterValue(mf, map[string]string{"template_id": "tmpl-1", "action": "preempted"})
	if value != 1 {
