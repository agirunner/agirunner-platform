...
func TestReconcile_PrePullFailed_EmitsErrorLog(t *testing.T) {
	...
	for _, e := range entries {
		if e.Operation == "container.pre_pull" && e.Status == "failed" {
			found = true
			if e.Error == nil || e.Error.Message != "registry unavailable" {
				t.Errorf("expected error 'registry unavailable', got %v", e.Error)
			}
			break
		}
	}
	if !found {
		t.Error("expected container.pre_pull error log entry emitted")
	}
}
