...
payload_text = os.environ.get("LIVE_TEST_OAUTH_SESSION_JSON", "").strip()
profile_id = os.environ.get("LIVE_TEST_OAUTH_PROFILE_ID", "").strip() or "openai-codex"
if not payload_text:
    raise SystemExit("[tests/live] LIVE_TEST_OAUTH_SESSION_JSON is required")
