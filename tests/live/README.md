...
- OAuth bootstrap SHOULD prefer exporting the current DB-backed session through `tests/live/export-current-oauth-session.sh` instead of maintaining a stale hand-copied session blob in `local.env`.
