...
export async function runHl2WorkflowControls(
  live: LiveContext,
  ...
  try {
    ...
    const workflow = await tenant.adminClient.createWorkflow({
      ...
      name: `HL-2 workflow controls ${live.runId}`,
    });
    ...
    const workflow2 = await tenant.adminClient.createWorkflow({
      template_id: template.id,
      name: `HL-2 full cancel ${live.runId}`,
    });

    await tenant.adminClient.cancelWorkflow(workflow2.id);
    const cancelledWorkflow = await tenant.adminClient.getWorkflow(workflow2.id);
    const cancellableStates = new Set(['cancelled', 'completed', 'failed']);
    if (!cancellableStates.has(cancelledWorkflow.state)) {
      throw new Error(`HL-2 expected cancelled/terminal workflow, got ${cancelledWorkflow.state}`);
    }
