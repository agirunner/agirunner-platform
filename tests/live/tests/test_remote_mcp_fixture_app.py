...
class RemoteMcpFixtureAppTests(unittest.TestCase):
    def test_search_research_corpus_returns_relevant_audit_export_documents_first(self) -> None:
        results = search_research_corpus("audit export reliability operator workflow", limit=3)

        self.assertGreaterEqual(len(results), 1)
        self.assertEqual("Enterprise Audit Export Reliability Benchmark", results[0]["title"])
