...
test_multi_orchestrator_script_survives_bootstrap_cleanup_of_artifact_dir() {
  local tmpdir envfile bootstrap_stub runner_stub artifacts_dir
  tmpdir="$(mktemp -d)"
  trap 'rm -rf "${tmpdir}"' RETURN
  envfile="${tmpdir}/env/local.env"
...
EOF
  ...
  make_stub "${bootstrap_stub}" '
rm -rf "${LIVE_TEST_ARTIFACTS_DIR}/multi-orchestrator-concurrent-assessment-workflows"
'
  make_stub "${runner_stub}" '
scenario_path="$1"
scenario="$(basename "${scenario_path%.json}")"
