...
describe('workflow config and instruction routes', () => {
  let db: TestDatabase;
  ...
  it('resolves workflow config, validates overrides, and assembles instruction layers on task claim', async () => {
    ...
    const taskId = workflow.json().data.tasks[0].id as string;

    const resolvedConfig = await app.inject({
      method: 'GET',
      url: `/api/v1/workflows/${workflowId}/config/resolved?show_layers=true`,
    ...
    expect(claimed.json().data.context.workflow.resolved_config).toEqual({
