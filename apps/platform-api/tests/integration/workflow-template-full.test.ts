...
describe('workflow + template full coverage', () => {
  let db: TestDatabase;
  ...
  beforeAll(async () => {
    ...
    artifactRoot = await mkdtemp(path.join(os.tmpdir(), 'agirunner-platform-state-'));
    const eventService = new EventService(db.pool);
    templateService = new TemplateService(db.pool, eventService);
    workflowService = new WorkflowService(db.pool, eventService, {
      ...config,
  ...
  it('covers FR-161/FR-169/FR-170/FR-171/FR-172/FR-177 workflow instantiation from template with dependencies', async () => {
    ...
    const [a, b] = workflow.tasks as Array<Record<string, unknown>>;
  ...
  it('persists task context_template into task context for execution-path contracts', async () => {
    ...
    const workflow = await workflowService.createWorkflow(admin, {
      template_id: template.id as string,
      name: 'context-contract-workflow',
  ...
  it('covers FR-162/FR-167 workflow status derivation and emitted events', async () => {
    const template = await templateService.createTemplate(admin, {
      name: 'state-template',
      slug: 'state-template',
    ...
    const loaded = await workflowService.getWorkflow(tenantId, workflow.id as string);

    expect(loaded.state).toBe('pending');

    const eventRows = await db.pool.query(
      `SELECT type FROM events WHERE tenant_id = $1 AND entity_id = $2 AND entity_type = 'workflow' ORDER BY created_at ASC`,
      [tenantId, workflow.id],
    );
    expect(eventRows.rows.some((row) => row.type === 'workflow.created')).toBe(true);
  });
  ...
  it('persists output_state declarations and enforces artifact/git/diff storage on completion', async () => {
    ...
    const workflow = await workflowService.createWorkflow(admin, {
      template_id: template.id as string,
      name: 'state-declaration-workflow',
      metadata: {
    ...
    const artifacts = await db.pool.query(
      'SELECT logical_path FROM workflow_artifacts WHERE tenant_id = $1 AND task_id = $2',
      [tenantId, task.id],
    );
  ...
  it('instantiates the SDLC output contract and exposes architect handoff via output_state and documents', async () => {
    ...
    const workflow = await workflowService.createWorkflow(admin, {
      template_id: template.id as string,
      name: 'sdlc-output-contract-workflow',
      parameters: {
