...
function runtimeDefaultQueryResult(sql: string, params?: unknown[]) {
  ...
  if (key === 'agent.orchestrator_loop_mode') {
    return { rowCount: 1, rows: [{ config_value: 'reactive' }] };
  }
  return { rowCount: 0, rows: [] };
}

describe('TaskClaimService', () => {
  it('reads orchestrator loop mode from runtime defaults when claiming orchestrator tasks', async () => {
    const client = {
    ...
    expect(
      client.query.mock.calls.some(([, params]) => Array.isArray(params) && params[1] === 'agent.orchestrator_loop_mode'),
    ).toBe(true);
  });
