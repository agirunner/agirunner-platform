...
describe('applyTaskCompletionSideEffects', () => {
  ...
  it('blocks the subject work item for an explicit blocked assessment decision', async () => {
      ...
      query: vi.fn(async (sql: string, params?: unknown[]) => {
        ...
        if (sql.includes('FROM task_handoffs')) {
          return {
            rows: [{
              completion: 'full',
              resolution: 'blocked',
              summary: 'The draft is blocked pending source verification.',
            }],
            rowCount: 1,
          };
        }
        if (sql.includes('SELECT p.definition')) {
          return {
            rows: [{
              definition: {
                process_instructions: 'Assessors can block unsafe draft revisions.',
        ...
        if (sql.includes('UPDATE workflow_work_items') && sql.includes("blocked_state = 'blocked'")) {
          ...
          expect(params).toEqual([
            ...
            'workflow-1',
            'work-item-author',
            'The draft is blocked pending source verification.',
            'blocked',
          ]);
          return { rows: [], rowCount: 1 };
        }
