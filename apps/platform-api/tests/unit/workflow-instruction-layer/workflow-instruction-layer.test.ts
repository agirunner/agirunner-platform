...
describe('buildWorkflowInstructionLayer', () => {
  ...
  it('builds orchestrator instructions with stage routing and visibility context', () => {
    ...
    expect(result?.content).toContain('qa-reviewer');
    expect(result?.content).toContain('request_changes');
    expect(result?.content).toContain('request_gate_approval');
    expect(result?.content).toContain('read_task_status');
