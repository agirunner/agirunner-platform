...
describe('buildWorkflowInstructionLayer', () => {
  ...
  it('builds orchestrator instructions with stage routing and visibility context', () => {
    ...
    expect(result?.content).toContain('If the current work item can close now and successor-stage routing or a gate request is still pending, apply that explicit mutation before submit_handoff instead of parking the closable work item and narrating the next wait state.');
