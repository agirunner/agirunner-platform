...
describe('buildWorkflowInstructionLayer', () => {
  ...
  it('builds orchestrator instructions with stage routing and visibility context', () => {
    ...
    expect(result?.content).toContain('If the current work_item_id is still in "triage", do not call create_task with stage_name "implementation" against that predecessor work item. Create or move the successor work item first, then dispatch the successor-stage task against that new work item.');
    ...
    expect(result?.content).toContain('If you plan to wait on a specific task id, call read_task_status on that exact task immediately before record_operator_brief or submit_handoff. Do not rely on an earlier read_task_output, read_work_item_continuity, or predecessor handoff to claim that task is still active.');
