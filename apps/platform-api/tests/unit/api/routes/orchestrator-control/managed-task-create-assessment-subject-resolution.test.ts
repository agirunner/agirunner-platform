...
describe('orchestratorControlRoutes', () => {
  ...
  it('rebinds review assessment linkage from the predecessor delivery handoff after same-activation stage progression', async () => {
    const reviewWorkItemId = '77777777-7777-4777-8777-777777777777';
    const implementationWorkItemId = '88888888-8888-4888-8888-888888888888';
    const createdTask = {
      id: 'task-reviewer',
      workflow_id: 'workflow-1',
      ...
      query: vi.fn(async (sql: string, params?: unknown[]) => {
        ...
        if (sql.includes('SELECT response') && sql.includes('workflow_tool_results')) {
          expect(params).toEqual([
            ...
            'workflow-1',
            'create_task',
            'create_review_assessment_from_parent_handoff',
          ]);
          return { rowCount: 0, rows: [] };
        }
        if (sql.includes("COALESCE(metadata->>'subject_task_id'")) {
    ...
    const pool = {
      query: vi.fn(async (sql: string, params?: unknown[]) => {
        if (sql.includes('FROM task_handoffs') && sql.includes('role_data->>\'subject_task_id\'')) {
          if (params?.[2] === reviewWorkItemId) {
            expect(params).toEqual(['tenant-1', 'workflow-1', reviewWorkItemId]);
            return { rowCount: 0, rows: [] };
          }
          if (params?.[2] === implementationWorkItemId) {
            expect(params).toEqual(['tenant-1', 'workflow-1', implementationWorkItemId]);
            return {
              rowCount: 1,
              rows: [{
        ...
        if (sql.includes('FROM tasks') && sql.includes('WHERE tenant_id = $1') && sql.includes('AND id = $2')) {
          ...
          return {
            ...
            rows: [{
              ...
              activation_id: 'activation-implement-progressed',
              assigned_agent_id: 'agent-1',
              is_orchestrator_task: true,
              state: 'in_progress',
            }],
        ...
        if (sql.includes('FROM workflow_work_items wi') && sql.includes('LEFT JOIN workflow_work_items parent')) {
          ...
          return {
            ...
            rows: [{
              id: reviewWorkItemId,
              stage_name: 'review',
              parent_work_item_id: implementationWorkItemId,
              parent_id: implementationWorkItemId,
              parent_stage_name: 'implement',
              workflow_lifecycle: 'planned',
            }],
          };
        }
        if (sql.includes('FROM workflows w') && sql.includes('LEFT JOIN workflow_activations wa')) {
          expect(params).toEqual(['tenant-1', 'workflow-1', 'activation-implement-progressed']);
          return {
            rowCount: 1,
            rows: [{
              lifecycle: 'planned',
    ...
    const response = await app.inject({
      ...
      headers: { authorization: 'Bearer test' },
      payload: {
        request_id: 'create_review_assessment_from_parent_handoff',
        title: 'Review audit export hang fix implementation',
        description: 'Review the implementation-ready advisory package after implement-stage completion.',
        work_item_id: reviewWorkItemId,
        stage_name: 'review',
        role: 'Code Reviewer',
        type: 'assessment',
    ...
    expect(taskService.createTask).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        metadata: expect.objectContaining({
          ...
          subject_revision: 1,
          created_by_orchestrator_task_id: 'task-orchestrator',
          orchestrator_activation_id: 'activation-implement-progressed',
        }),
      }),
      client,
    );
