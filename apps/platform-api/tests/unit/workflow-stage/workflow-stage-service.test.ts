...
describe('WorkflowStageService', () => {
  ...
  it('marks planned stages completed when all work items are already complete', async () => {
    const pool = {
      query: vi
        .fn()
        .mockResolvedValueOnce({ rowCount: 1, rows: [{ id: 'workflow-1' }] })
        .mockResolvedValueOnce({
          ...
          rows: [
            ...
            },
            {
              ...
              total_work_item_count: 3,
              first_work_item_at: new Date('2026-03-11T01:00:00Z'),
              last_completed_work_item_at: new Date('2026-03-11T01:45:00Z'),
            },
          ],
        }),
    };
    ...
    expect(stages).toEqual([
      ...
      expect.objectContaining({
        ...
        status: 'completed',
        is_active: false,
        completed_at: '2026-03-11T01:45:00.000Z',
      }),
    ]);
  });
});
