...
describe('WorkflowRailService live query filters', () => {
  it('does not pin a selected workflow when a server-driven needs-action filter excludes it', async () => {
    ...
    expect(result.rows).toEqual([expect.objectContaining({ workflow_id: 'workflow-needs-action' })]);
  });
  ...
  it('does not pin a selected workflow when playbook or recency filters exclude it', async () => {
    ...
    expect(result.rows).toEqual([expect.objectContaining({ workflow_id: 'workflow-visible' })]);
  });
