...
describe('milestone c workflow/template e2e', () => {
  let db: TestDatabase;
  ...
  beforeAll(async () => {
    ...
    const register = await app.inject({
      ...
      payload: { name: 'workflow-agent', capabilities: ['typescript'] },
    });
  ...
  it('runs template CRUD and full workflow lifecycle', async () => {
    ...
    expect(createWorkflow.json().data.template_id).toBe(templateV2Id);
    expect(createWorkflow.json().data.template_version).toBe(2);
    expect((createWorkflow.json().data.tasks as Array<{ title: string }>)[0].title).toBe('Analyze v2 login');
    const workflowId = createWorkflow.json().data.id as string;
    ...
    expect(context.json().data.workflow.variables.feature).toBe('login');
    ...
    expect(getWorkflow.json().data.state).toBe('completed');
