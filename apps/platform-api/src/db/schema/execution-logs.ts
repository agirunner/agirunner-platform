...
export const executionLogs = pgTable(
  ...
  (table) => [
    ...
    index('idx_exlogs_category_op').on(table.tenantId, table.category, table.operation, table.createdAt),
    ...
    index('idx_exlogs_resource').on(table.tenantId, table.resourceType, table.resourceId, table.createdAt),
