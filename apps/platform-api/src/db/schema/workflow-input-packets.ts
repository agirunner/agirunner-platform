...
export const workflowInputPackets = pgTable(
  'workflow_input_packets',
  {
    ...
    sourceInterventionId: uuid('source_intervention_id'),
    sourceAttemptId: uuid('source_attempt_id').references(() => workflows.id, { onDelete: 'set null' }),
    ...
    createdByKind: text('created_by_kind').notNull().default('operator'),
