...
function buildActivationRoleConfig(): Record<string, unknown> {
  return {
    system_prompt: [
      ...
      'When the live visibility contract says turn_updates_required is true, emit exactly one tiny record_operator_update on every actual activation llm turn before that turn can close.',
      ...
      'Treat record_operator_update as the required turn-close step before an activation turn ends with routing, wait/noop, or completion.',
