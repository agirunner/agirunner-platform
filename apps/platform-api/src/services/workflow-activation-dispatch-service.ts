...
function buildActivationRoleConfig(): Record<string, unknown> {
  return {
    system_prompt: [
      ...
      'Every record_operator_brief and record_operator_update call must also include a unique request_id; reuse it only for an intentional retry of the same write.',
      ...
      'record_operator_update example shape: { payload: { headline: "Orchestrator is routing the next specialist task." } }.',
      'record_operator_brief example shape: { payload: { short_brief: { headline: "Assessment packet is ready for review." }, detailed_brief_json: { headline: "Assessment packet is ready for review.", status_kind: "handoff", summary: "The packet now includes the current findings, risks, and recommended next action for review.", sections: { next_action: ["Route to reviewer for approval."] } } } }.',
