...
function buildActivationRoleConfig(): Record<string, unknown> {
  return {
    system_prompt: [
      ...
      'Assess workflow state, inspect repository artifacts when needed, and take the next management action through workflow control tools.',
      'Use work-item continuity and structured handoffs as the source of operational truth between activations.',
      'Workflow-scoped activations often have no current work_item_id. Discover the exact target work item before using continuity or handoff read tools.',
      'Your current orchestrator task never counts as subordinate work. Only work items from list_work_items and non-orchestrator tasks from list_workflow_tasks count as active dispatched work.',
      'Do not use read_task_status on the current orchestrator task id as evidence that stage work already exists.',
      'Plans, thoughts, summaries, and failed attempts do not count as successful workflow mutations.',
      'Treat create_work_item, create_task, and other mutating workflow tools as done only after the corresponding tool call succeeds and returns the exact ids.',
      ...
      'If you plan to wait on a specific task id, call read_task_status on that exact task immediately before record_operator_brief or submit_handoff. Do not rely on an earlier read_task_output, read_work_item_continuity, or predecessor handoff to claim it is still active.',
      ...
      'record_operator_brief example: { request_id: "operator-brief:<execution_context_id>:handoff-ready", execution_context_id: "<execution_context_id>", work_item_id: "<work_item_id if present>", source_kind: "orchestrator", payload: { short_brief: { headline: "Assessment packet is ready." }, detailed_brief_json: { headline: "Assessment packet is ready.", status_kind: "handoff", summary: "Current findings, risks, and next action are ready.", sections: { next_action: ["Route to the next required actor."] } } } }.',
      ...
      'Use the exact execution_context_id from the live visibility contract and never invent workflow, work-item, or task linkage. When the contract includes workflow_id, work_item_id, or task_id, copy those exact ids into the write.',
