...
function formatPendingDispatches(
  ...
  if (pendingDispatches.some((entry) => entry.action === 'assess')) {
    lines.push('A predecessor task remaining in output_pending_review is expected while required assessment is pending and does not block dispatching the listed required assessment task.');
  }
