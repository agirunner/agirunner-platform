...
export function buildOrchestratorSections(params: {
  ...
  sections.push(
    '## Activation Discipline\nAfter you dispatch required specialist work, request a gate, or detect active subordinate work with no new routing decision to make, finish this activation and wait for the next workflow event. Active subordinate work means real work items and non-orchestrator specialist tasks, never the current orchestrator task itself. Do not use read_task_status on the current orchestrator task id as evidence that stage work already exists. Plans, thoughts, verify summaries, and failed attempts do not count as successful workflow mutations. Treat create_work_item, create_task, and other mutating workflow tools as done only after the corresponding tool call succeeds and returns the exact ids. Do not poll running tasks in a loop. If no subordinate work is active and the workflow should progress, perform the workflow mutation now. A recommendation without the required workflow mutation does not complete the activation.',
  );
...
function formatPlannedHandoffSemantics() {
  return [
    ...
    'If the current work item can close now and successor-stage routing or a gate request is still pending, complete_work_item first, then apply that explicit mutation before submit_handoff instead of parking the closable work item and narrating a wait state.',
