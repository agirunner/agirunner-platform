...
const entries: SafetynetEntry[] = [
  {
    ...
    intervention: 'platform derives missing subject linkage from explicit context, predecessor delivery lineage on the target work-item chain, or activation lineage',
  ...
  },
  {
    ...
    default_policy: 'enabled',
    disposition: 'keep',
    trigger: 'handoff submission still contains task-local output references or replay payload state that requires canonical repair before persistence',
    nominal_contract: 'agent submits stable handoff references and an attempt-correct replay payload without needing platform-side repair',
    intervention: 'platform repairs task-local handoff references into stable operator-facing references when safe and reconciles editable replay state',
    risk_if_triggered: 'handoff success can hide that the submitted payload still used unstable output references or replayed imperfectly',
    operator_visibility: 'must remain reviewable in the safetynet catalog and owner code',
    ...
    log_event_type: 'platform.safetynet.triggered',
    review_notes: 'keep narrow and visible; repair may canonicalize safe output references and replay state only, never routine task-owned linkage persistence or invented workflow meaning',
    status: 'candidate_for_tightening',
  },
  {
    kind: 'safetynet_behavior',
