...
const entries: SafetynetEntry[] = [
  ...
  },
  {
    ...
    default_policy: 'enabled',
    disposition: 'keep',
    trigger: 'handoff submission requires normalization or replay reconciliation to remain legal',
    nominal_contract: 'agent submits a fully normalized, attempt-correct handoff payload',
    intervention: 'platform normalizes handoff state and reconciles editable replay state',
    risk_if_triggered: 'handoff success can hide that the submitted payload was incomplete or replayed imperfectly',
    operator_visibility: 'must remain reviewable in the safetynet catalog and owner code',
    ...
    log_event_type: 'platform.safetynet.triggered',
    review_notes: 'keep narrow and visible; do not expand normalization semantics casually',
    status: 'candidate_for_tightening',
  },
  {
    kind: 'safetynet_behavior',
