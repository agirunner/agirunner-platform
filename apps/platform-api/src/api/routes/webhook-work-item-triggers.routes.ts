...
export const webhookWorkItemTriggerRoutes: FastifyPluginAsync = async (app) => {
  const triggerService = app.webhookWorkItemTriggerService;

  app.post('/api/v1/work-item-triggers', { preHandler: [authenticateApiKey, withScope('admin')] }, async (request, reply) => {
  ...
  app.get('/api/v1/work-item-triggers', { preHandler: [authenticateApiKey, withScope('admin')] }, async (request) => {
    return triggerService.listTriggers(request.auth!.tenantId);
  });

  app.patch('/api/v1/work-item-triggers/:id', { preHandler: [authenticateApiKey, withScope('admin')] }, async (request) => {
  ...
  app.delete('/api/v1/work-item-triggers/:id', { preHandler: [authenticateApiKey, withScope('admin')] }, async (request) => {
    const params = request.params as { id: string };
  ...
  app.post(
    '/api/v1/work-item-triggers/:id/invoke',
    {
