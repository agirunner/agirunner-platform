...
export const missionControlRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/mission-control/live', auth, async (request) => {
    ...
    return {
      data: await app.missionControlLiveService.getLive(request.auth!.tenantId, {
  ...
  app.get('/api/v1/mission-control/recent', auth, async (request) => {
    ...
    return {
      data: await app.missionControlRecentService.getRecent(request.auth!.tenantId, {
  ...
  app.get('/api/v1/mission-control/history', auth, async (request) => {
    ...
    return {
      data: await app.missionControlHistoryService.getHistory(request.auth!.tenantId, {
  ...
  app.get('/api/v1/mission-control/workflows/:id/workspace', auth, async (request) => {
    ...
    return {
      data: await app.missionControlWorkspaceService.getWorkspace(request.auth!.tenantId, params.id, {
