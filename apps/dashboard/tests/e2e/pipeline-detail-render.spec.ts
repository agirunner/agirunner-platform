...
const PIPELINE_ID = 'pipeline-active';
...
test('renders pipeline detail for a valid active pipeline id', async ({ page }) => {
  await page.goto('/login');

  await page.getByLabel('API Key').fill('ab_admin_example_key');
  await page.getByRole('button', { name: 'Sign in' }).click();

  await page.getByRole('link', { name: 'Active Pipeline' }).click();
  ...
  await page.screenshot({
    path: '../../tests/artifacts/integration/screenshots/issue-74-pipeline-detail-post-fix.png',
    fullPage: true,
  });
});
