...
function readSource() {
  return ['./role-definitions-page.tsx', '../../components/list-pagination.tsx']
...
function readCombinedSource() {
  return [
    ...
    readFileSync(
      resolve(import.meta.dirname, './role-definitions-orchestrator.sections.tsx'),
      'utf8',
    ),
    readFileSync(
      resolve(import.meta.dirname, './role-definitions-orchestrator.dialogs.tsx'),
      'utf8',
    ),
    readFileSync(
      resolve(import.meta.dirname, './role-definitions-orchestrator.dialog-shared.tsx'),
      'utf8',
    ),
    readFileSync(
      resolve(import.meta.dirname, './role-definitions-orchestrator.pool-dialog.tsx'),
      'utf8',
    ),
    readFileSync(resolve(import.meta.dirname, './role-definitions-orchestrator.form.ts'), 'utf8'),
    readFileSync(
      resolve(import.meta.dirname, './role-definitions-orchestrator.support.ts'),
      'utf8',
    ),
...
describe('role definitions page source', () => {
  ...
  it('keeps the role editor dialog scrollable and wide enough for large forms', () => {
    ...
    const dialogSource = readFileSync(
      resolve(import.meta.dirname, './role-definitions-dialog.tsx'),
      'utf8',
    );
    expect(dialogSource).toContain(
      'top-[5vh] flex max-h-[90vh] max-w-[68rem] translate-y-0 flex-col overflow-hidden p-0',
    );
  ...
  it('exposes structured model and active-state controls in the dialog', () => {
    ...
    expect(source).not.toContain('Create skill');
    expect(source).not.toContain('Edit skill');
    expect(source).not.toContain('Archive skill');
    expect(source).not.toContain('Restore skill');
    expect(source).not.toContain('Skill library');
  ...
  it('preselects verified remote MCP servers that are enabled by default for new specialists', () => {
    const source = readCombinedSource();

    expect(source).toContain('listDefaultRoleMcpServerIds');
    expect(source).toContain('defaultRemoteMcpServerIds');
    expect(source).toContain('createRoleForm(null, defaultToolIds, defaultRemoteMcpServerIds)');
    expect(source).toContain('enabled_by_default_for_new_specialists');
  });
  ...
  it('adds a first-class orchestrator control plane with direct prompt, model, and pool editing', () => {
    ...
    expect(source).not.toContain(
      'Existing worker names stay fixed. Create a new orchestrator entry here if you need a different name.',
    );
    ...
    expect(source).not.toContain(
      'grid gap-2 rounded-lg border border-border/70 bg-background/80 p-3',
    );
    ...
    expect(dialogsSource).not.toContain(
