...
import { STRUCTURED_CONTROL_UNSET_VALUE } from './playbook-authoring-structured-choice-controls.js';
...
export function TypedParameterValueControl(props: {
  ...
  if (props.valueType === 'boolean') {
    return (
      <Select
        value={props.value || STRUCTURED_CONTROL_UNSET_VALUE}
        onValueChange={(value) =>
          props.onChange(value === STRUCTURED_CONTROL_UNSET_VALUE ? '' : value)
        }
      >
        <SelectTrigger>
        ...
        <SelectContent>
          <SelectItem value={STRUCTURED_CONTROL_UNSET_VALUE}>Unset</SelectItem>
  ...
  return (
    <div className="space-y-3 rounded-xl border border-border/70 bg-muted/10 p-4">
      ...
      {editorState.entries.length === 0 ? (
        ...
        editorState.entries.map((entry, index) => (
          <StructuredParameterEntryCard
            ...
            onRemove={() =>
              updateEntries((current) => current.filter((_, entryIndex) => entryIndex !== index))
            }
          />
        ))
      )}
      <Button
        type="button"
        variant="outline"
        onClick={() => updateEntries((current) => [...current, createStructuredParameterEntry()])}
      >
        ...
        {props.valueType === 'object' ? (
          <label className="grid gap-1 text-xs">
            ...
            <Input
              ...
              className={props.errors.key ? 'border-red-300 focus-visible:ring-red-500' : undefined}
...
function StructuredScalarValueField(props: {
  ...
  if (props.entry.valueType === 'boolean') {
    return (
      <Select
        value={props.entry.value || STRUCTURED_CONTROL_UNSET_VALUE}
        onValueChange={(value) =>
          props.onChange(value === STRUCTURED_CONTROL_UNSET_VALUE ? '' : value)
        }
      >
