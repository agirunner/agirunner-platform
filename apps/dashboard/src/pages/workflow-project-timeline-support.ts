...
export interface WorkflowProjectTimelineMetric {
  label: string;
  value: string;
}

export interface WorkflowProjectTimelineOverview {
  metrics: WorkflowProjectTimelineMetric[];
  summary: string;
}

export interface WorkflowProjectTimelinePacket {
  workflowId: string;
  workflowName: string;
  workflowHref: string;
...
export function buildWorkflowProjectTimelineOverview(
  entries: DashboardProjectTimelineEntry[],
): WorkflowProjectTimelineOverview {
...
export function buildWorkflowProjectTimelinePacket(
  entry: DashboardProjectTimelineEntry,
): WorkflowProjectTimelinePacket {
  return {
    ...
    ].filter((metric): metric is WorkflowProjectTimelineMetric => metric !== null),
  };
}

function describePacketSummary(entry: DashboardProjectTimelineEntry): string {
...
function describeNextAction(entry: DashboardProjectTimelineEntry): string {
  if (isFailureState(entry.state)) {
...
function describeCompletedLabel(entry: DashboardProjectTimelineEntry): string {
...
): WorkflowProjectTimelineMetric | null {
