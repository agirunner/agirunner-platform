...
function readSource() {
  return readFileSync(resolve(import.meta.dirname, './workspace-artifact-explorer-layout.tsx'), 'utf8');
}

describe('workspace artifact explorer adaptive layout source', () => {
  ...
  it('keeps the desktop dual-pane layout for large screens', () => {
    ...
    expect(source).toContain('WorkspaceArtifactQuickInspector');
  });
});
