export interface WorkspaceArtifactScopeChip {
  label: string;
  value: string;
}

export function buildWorkspaceArtifactScopeChips(input: {
  query: string;
  ...
  createdFrom: string;
  createdTo: string;
}): WorkspaceArtifactScopeChip[] {
  const chips: WorkspaceArtifactScopeChip[] = [];
  if (input.query.trim()) {
...
export function describeWorkspaceArtifactNextAction(input: {
