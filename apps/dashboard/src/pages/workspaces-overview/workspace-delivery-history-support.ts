...
export interface WorkspaceDeliveryQuestionPacket {
  label: string;
  value: string;
  detail: string;
}

export interface WorkspaceDeliveryPacket {
  workflowId: string;
  workflowName: string;
  workflowHref: string;
...
export interface WorkspaceDeliveryAttentionOverview {
  summary: string;
  nextActionHref: string;
  packets: WorkspaceDeliveryQuestionPacket[];
}

export interface WorkspaceDeliveryAttentionState {
  statusLabel: string;
...
export function buildWorkspaceDeliveryAttentionOverview(
  entries: DashboardWorkspaceTimelineEntry[],
): WorkspaceDeliveryAttentionOverview {
...
export function buildWorkspaceDeliveryPacket(
  entry: DashboardWorkspaceTimelineEntry,
): WorkspaceDeliveryPacket {
  return {
    ...
    signals: buildWorkspaceDeliverySignals(entry),
  };
}

export function buildWorkspaceDeliveryAttentionState(
  entry: DashboardWorkspaceTimelineEntry,
): WorkspaceDeliveryAttentionState {
...
function buildLatestRunPacket(
  latestRun: DashboardWorkspaceTimelineEntry | undefined,
): WorkspaceDeliveryQuestionPacket {
  if (!latestRun) {
    return {
      label: 'What ran',
      value: 'No recent run',
      detail: 'No workspace delivery runs are available yet.',
    };
  }
...
function buildFailurePacket(
  failedRun: DashboardWorkspaceTimelineEntry | undefined,
  failureCount: number,
): WorkspaceDeliveryQuestionPacket {
  if (!failedRun) {
    return {
...
function buildAttentionPacket(
  attentionRuns: DashboardWorkspaceTimelineEntry[],
): WorkspaceDeliveryQuestionPacket {
...
function buildInspectPacket(
  inspectTarget: DashboardWorkspaceTimelineEntry | undefined,
): WorkspaceDeliveryQuestionPacket {
  if (!inspectTarget) {
    return {
      label: 'Inspect next',
...
function buildWorkspaceDeliverySignals(entry: DashboardWorkspaceTimelineEntry): string[] {
  return [
    ...
    .filter((signal): signal is WorkspaceDeliverySignalCandidate => signal !== null)
...
): WorkspaceDeliverySignalCandidate | null {
...
function buildAttentionSummary(
  ...
  if (!latestRun) {
    return 'No workspace runs are available yet.';
  }
  if (!inspectTarget) {
...
function describeInspectPriority(entry: DashboardWorkspaceTimelineEntry): string {
  if (isFailureState(entry.state)) {
...
function sortByCreated(entries: DashboardWorkspaceTimelineEntry[]): DashboardWorkspaceTimelineEntry[] {
  return [...entries].sort(
...
function formatRunGroupValue(entries: DashboardWorkspaceTimelineEntry[]): string {
  if (entries.length === 0) {
    return 'All clear';
  }
  if (entries.length === 1) {
...
function countFailures(entries: DashboardWorkspaceTimelineEntry[]): number {
...
function buildInspectorHref(entry: DashboardWorkspaceTimelineEntry | undefined): string {
  if (!entry) {
    return '#';
  }
...
function requiresAttention(entry: DashboardWorkspaceTimelineEntry): boolean {
  return (
