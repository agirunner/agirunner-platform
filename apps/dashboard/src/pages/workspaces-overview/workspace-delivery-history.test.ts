...
function readSource() {
  return ['./workspace-delivery-history.tsx', './workspace-delivery-history-support.ts']
    .map((path) => readFileSync(resolve(import.meta.dirname, path), 'utf8'))
    .join('\n');
}

describe('workspace delivery history source', () => {
  it('renders question-driven overview packets, compact run cards, and direct operator actions', () => {
    ...
    expect(source).toContain('dashboardApi.getWorkspaceTimeline(workspaceId)');
    expect(source).toContain('buildWorkspaceDeliveryAttentionOverview');
    expect(source).toContain('buildWorkspaceDeliveryAttentionState');
    expect(source).toContain('buildWorkspaceDeliveryPacket');
    expect(source).toContain('Workspace delivery timeline');
    ...
    expect(source).not.toContain('Workspace delivery is being rebuilt');
