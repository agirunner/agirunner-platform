...
describe('logs page source', () => {
  ...
  it('keeps the inspector surfaces available without forcing inspector summary cards onto raw logs', () => {
    ...
    expect(source).toContain('dashboardApi.listWorkspaces()');
    expect(source).toContain('dashboardApi.listWorkflows(requestFilters)');
    expect(source).toContain('dashboardApi.listTasks(requestFilters)');
    ...
    expect(source).toContain("queryKey: ['operator-log', 'filter-workspaces']");
    expect(source).toContain("queryKey: ['operator-log', 'filter-workflows', filters.workspace, logScope]");
    expect(source).toContain("queryKey: ['operator-log', 'filter-tasks', filters.workflow, logScope]");
    expect(source).toContain('const operationItemsOverride = useMemo(');
    expect(source).toContain('const roleItemsOverride = useMemo(');
    expect(source).toContain('const actorItemsOverride = useMemo(');
    expect(source).toContain('toOperationItems(operationsQuery.data)');
    expect(source).toContain('toRoleItems(rolesQuery.data)');
    expect(source).toContain('toActorItems(actorsQuery.data)');
    expect(source).toContain('operationItemsOverride={operationItemsOverride}');
    expect(source).toContain('roleItemsOverride={roleItemsOverride}');
    expect(source).toContain('actorItemsOverride={actorItemsOverride}');
    ...
    expect(source).toContain("const operationsQuery = useQuery({");
    expect(source).toContain("const rolesQuery = useQuery({");
    expect(source).toContain("const actorsQuery = useQuery({");
    expect(source).toContain("const workspacesQuery = useQuery({");
    expect(source).toContain("const workflowsQuery = useQuery({");
    expect(source).toContain("const tasksQuery = useQuery({");
