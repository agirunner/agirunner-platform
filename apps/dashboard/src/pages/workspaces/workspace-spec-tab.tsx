...
import { summarizeWorkspaceContext } from './workspace-settings-support.js';

export function WorkspaceSpecTab(props: {
  ...
  const contextSummary = summarizeWorkspaceContext(props.workspaceContext);
        ...
        <CardContent className="space-y-6 p-4">
          <div className="space-y-2">
            ...
            <p className="text-sm leading-6 text-muted">
              Reusable workspace context for playbooks. This is the place for durable LLM context that
              should flow into workflow inputs.
            </p>
            <p className="max-w-3xl text-sm leading-5 text-muted">{contextSummary}</p>
            <Textarea
              value={props.workspaceContext}
              className="min-h-[160px]"
              onChange={(event) => props.onWorkspaceContextChange(event.target.value)}
            />
            <p className="text-sm leading-6 text-muted">
              Use this for stable workspace context. It is separate from the workspace description and
              can be mapped by playbooks into workflow inputs.
            </p>
          </div>

          <div className="space-y-2 border-t border-border/70 pt-4">
            <CardTitle className="text-base">Workspace Knowledge</CardTitle>
            <p className="text-sm leading-6 text-muted">
              Edit curated workspace facts and policies as simple key/value entries instead of managing
            ...
            <p className="max-w-3xl text-sm leading-5 text-muted">
              Playbooks can map these keys through <code>workspace.settings.knowledge.&lt;key&gt;</code>.
            </p>
          </div>

          <StructuredEntryEditor
            title="Key/Value pairs"
            description="Use simple string or JSON values for reusable workspace knowledge."
