...
describe('workspace content tables source', () => {
  it('uses responsive packets for workflow documents instead of a desktop-only table', () => {
    const tablesSource = readSource('./workspace-content-tables.tsx');
    const recordsSource = readSource('./workspace-content-document-records.tsx');
  ...
  it('keeps artifacts operator-managed with responsive cards and delivery packets', () => {
    const tablesSource = readSource('./workspace-content-tables.tsx');
    const recordsSource = readSource('./workspace-content-artifact-records.tsx');
