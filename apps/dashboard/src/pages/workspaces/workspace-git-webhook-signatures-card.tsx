...
export function WorkspaceGitWebhookSignaturesCard({
  workspace,
  compact = false,
}: {
  ...
  const hasRepository = Boolean(workspace.repository_url);
  const [provider, setProvider] = useState(workspace.git_webhook_provider ?? 'github');
  ...
  const secretError =
    trimmedSecret.length > 0 && trimmedSecret.length < 8
      ...
      : null;
  const currentProvider = workspace.git_webhook_provider
    ? formatProviderName(workspace.git_webhook_provider)
    : 'Not set';

  const mutation = useMutation({
    mutationFn: (payload: { provider: string; secret: string }) =>
      dashboardApi.configureGitWebhook(workspace.id, payload),
    onSuccess: () => {
      ...
      <CardContent className={compact ? 'space-y-3 p-4 pt-0' : 'space-y-3 p-4'}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-sm leading-6 text-muted">
              {buildGitSignatureSummary(workspace, hasRepository)}
            </p>
          </div>
          <Button
            variant="outline"
        ...
        <div className="flex flex-wrap gap-2">
          <SignalPill
            ...
            variant={hasRepository && workspace.git_webhook_provider ? 'success' : 'secondary'}
          />
          <SignalPill
            label="Secret"
            value={
              hasRepository
                ? workspace.git_webhook_secret_configured
                  ? 'Configured'
                  : 'Missing'
                : 'Not in use'
            }
        ...
        {hasRepository ? (
          <p className="break-all text-xs leading-5 text-muted">
            <span className="font-medium text-foreground">Repository:</span> {workspace.repository_url}
          </p>
        ) : null}
        ...
        <div className="rounded-xl border border-border/70 bg-muted/30 px-3 py-3 text-sm leading-6 text-muted">
          ...
          {buildGitSignatureNextAction(workspace, hasRepository)}
        </div>
          ...
          <section className="space-y-4 rounded-xl border border-border/70 bg-background/70 p-4">
            ...
            <div className="flex flex-col gap-3 rounded-xl border border-border/70 bg-muted/30 p-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              >
                ...
                {workspace.git_webhook_secret_configured ? 'Update secret' : 'Configure secret'}
              </Button>
            </div>
          </section>
        ) : null}
...
function buildGitSignatureSummary(
  ...
  if (!hasRepository) {
    return 'Optional. Use this only when GitHub, Gitea, or GitLab should be allowed to deliver signed inbound repository events for this workspace.';
  }
  if (workspace.git_webhook_secret_configured) {
  ...
  return 'Repository is linked. Configure signatures only if this workspace should trust GitHub, Gitea, or GitLab inbound hooks.';
}
...
function buildGitSignatureNextAction(
  ...
  if (!hasRepository) {
    return 'Leave this collapsed unless GitHub, Gitea, or GitLab inbound hooks should be trusted for this workspace.';
  }
  if (!workspace.git_webhook_provider || !workspace.git_webhook_secret_configured) {
