...
import { WorkspaceGitWebhookSignaturesCard } from './workspace-git-webhook-signatures-card.js';

type EditorTarget = 'create' | DashboardWebhookWorkItemTriggerRecord;

export function WebhookTriggersCard({ workspace }: { workspace: DashboardWorkspaceRecord }): JSX.Element {
  ...
  const triggersQuery = useQuery({
    queryKey: ['webhook-work-item-triggers', workspace.id],
  ...
  const invalidateTriggers = () =>
    queryClient.invalidateQueries({ queryKey: ['webhook-work-item-triggers', workspace.id] });
  ...
  const workspaceTriggers = useMemo(
    () =>
  ...
  const triggerSummary =
    workspaceTriggers.length === 0
      ...
      : `${workspaceTriggers.length} ${workspaceTriggers.length === 1 ? 'inbound hook' : 'inbound hooks'} configured.`;
  ...
  return (
    <Card className="border-border/70 shadow-none">
        ...
        >
          <div className="space-y-1.5">
            ...
            <p className="text-sm leading-6 text-muted">
              Turn external workspace events into workflow work when needed.
            </p>
      ...
      <CardContent className="space-y-6 border-t border-border/70 p-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted">
                Inspect, toggle, or revise workspace-scoped webhook rules.
              </p>
            </div>
        ...
        <WorkspaceGitWebhookSignaturesCard workspace={workspace} compact />
      </CardContent>
      ) : null}
    </Card>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-md border py-12 text-center">
      <div className="space-y-1">
        <p className="font-medium">No webhook triggers for this workspace</p>
        <p className="text-sm text-muted">
          Add the first inbound hook when this workspace should turn external events into workflow work.
        </p>
      </div>
