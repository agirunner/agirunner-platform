...
export interface WorkspaceWorkflowOption {
  id: string;
  name: string;
  state: string;
  createdAt?: string;
...
export interface WorkspaceWorkItemOption {
  id: string;
  workflowId: string | null;
  title: string;
  stageName: string;
...
export function normalizeWorkspaceList(
  response: { data: DashboardWorkspaceRecord[] } | DashboardWorkspaceRecord[] | undefined,
  ...
  return records
    .filter((workspace): workspace is DashboardWorkspaceRecord => isRecord(workspace))
    .map((workspace) => {
      const id = readContentString(workspace.id, '');
      const slug = readContentString(workspace.slug, id || 'workspace');
      return {
        ...workspace,
        id,
        slug,
        name: readContentString(workspace.name, slug || id || 'Unnamed workspace'),
      };
    })
...
export function buildWorkflowOptions(
  timeline: DashboardWorkspaceTimelineEntry[] | undefined,
  ...
  const workflows: WorkspaceWorkflowOption[] = [];
...
export function normalizeTaskOptions(response: unknown): WorkspaceTaskOption[] {
  ...
  const tasks: WorkspaceTaskOption[] = [];
...
export function normalizeDocumentRecords(
  ...
  for (const document of documents) {
    ...
    normalized.push({
      ...
      scope: document.scope === 'workspace' ? 'workspace' : 'workflow',
...
export function normalizeArtifactRecords(
  ...
  for (const artifact of artifacts) {
    ...
    normalized.push({
      ...
      workspace_id: readOptionalContentString(artifact.workspace_id),
...
export function summarizeArtifactExecutionScope(input: {
  selectedWorkflow: WorkspaceWorkflowOption | null;
  selectedWorkItem: WorkspaceWorkItemOption | null;
  selectedTask: WorkspaceTaskOption | null;
