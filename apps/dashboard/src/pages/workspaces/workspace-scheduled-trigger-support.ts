...
export function buildScheduledTriggerOverview(
  ...
  if (triggers.length === 0) {
    return {
      ...
      summary:
        'Scheduled work-item automation is empty for this workspace. Add the first schedule to create recurring work and wake the orchestrator through the normal activation flow.',
      nextAction:
        'Create the first schedule, pick the target workflow, and define the work item title before you leave this workspace.',
      packets: [
        {
          label: 'Schedule coverage',
          value: '0 schedules',
          detail: 'No scheduled work-item automation is active for this workspace yet.',
        },
        {
  ...
  return {
    ...
    summary:
      dueCount > 0
        ...
        : `${activeCount} active schedule${activeCount === 1 ? ' is' : 's are'} set to keep this workspace moving without manual launch steps.`,
    nextAction:
      dueCount > 0
        ...
        : disabledCount > 0
          ...
          : 'Check the next upcoming schedule, then edit interval, daily timing, or work-item routing only if the workspace rhythm has changed.',
    packets: [
      {
        label: 'Schedule coverage',
