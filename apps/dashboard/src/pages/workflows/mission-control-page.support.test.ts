...
describe('mission control page support', () => {
  ...
  it('builds a canonical shell href from partial state', () => {
    expect(
      ...
      '/workflows?mode=recent&rail=workflow&lens=tasks&workflow=workflow-2&tab=board&view=release-train&scope=workspace%3Aalpha',
    );
    expect(buildMissionControlShellHref({ rail: 'attention' })).toBe('/workflows');
  });
