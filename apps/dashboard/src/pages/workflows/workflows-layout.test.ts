...
describe('buildWorkflowsShellClassName', () => {
  it('uses a single-column shell when the workflow rail is hidden', () => {
    ...
    expect(buildWorkflowsShellClassName(true)).toContain('min-h-[calc(100dvh-5.75rem)]');
    expect(buildWorkflowsShellClassName(true)).toContain('lg:h-[calc(100dvh-3.5rem)]');
  ...
  it('uses the two-column shell when the workflow rail is visible', () => {
    ...
    expect(buildWorkflowsShellClassName(false)).toContain('min-h-[calc(100dvh-5.75rem)]');
    expect(buildWorkflowsShellClassName(false)).toContain('lg:h-[calc(100dvh-3.5rem)]');
  ...
  it('builds a stable board/workbench split that keeps the board large enough for stacked work-item cards', () => {
    ...
    expect(buildWorkflowWorkspaceSplitClassName()).not.toContain('minmax(20rem,var(--workflow-board-track))');
    expect(buildWorkflowWorkspaceSplitClassName()).not.toContain('minmax(20rem,var(--workflow-workbench-track))');
