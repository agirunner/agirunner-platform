...
export function WorkflowLiveConsoleEntry(props: {
  ...
  return (
    <article
      ...
      className={`grid min-w-0 grid-cols-[max-content_minmax(0,1fr)_max-content] items-baseline gap-3 border-b border-slate-950/90 px-4 py-2.5 font-mono text-sm leading-6 text-slate-100 last:border-b-0 ${entryStyle.entryClassName}`}
    >
      <span className={`self-start ${entryStyle.promptClassName}`}>&gt;</span>
      <p
        className="min-w-0 truncate pr-2 text-slate-100"
